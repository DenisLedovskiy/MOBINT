...
class CustomDecodableSerializer<T: Decodable>: DataResponseSerializerProtocol {
    ...
    func serialize(request: URLRequest?, response: HTTPURLResponse?, data: Data?, error: Error?) throws -> T {
        ...
        do {
            ...
            let value = try JSONDecoder().decode(APIError.self, from: dataResponse)
            if value.message != nil {
                let customError = errorParser.parseCustomError(value.message)
                throw customError
            } else {
                let customError = errorParser.parse(error, statusCode: response?.statusCode ?? 0)
                throw customError
            }
        }
    }
